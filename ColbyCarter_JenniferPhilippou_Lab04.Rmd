...
For the crimes per person, probability of arrest, probability of conviction, average sentance days ... we see a right skew and correct for it by transforming the variable with the square root function.
...
# pct young male: right skew with one extreme outlier at right 10 percentage points above the next highest value. Remove record with top coded variable (like done in the batting averages example 13.12)
```{r, fig.width = 10}
par(mfrow = c(2,3))
...
```
In case you want to see histograms of other variables
```{r, fig.width = 10}
par(mfrow=c(2,3))
for(i in (3:26)){
   x = raw_data[,i]
   df_name = names(raw_data)
   df_name = df_name[i] 
hist(x, main = df_name )}
```
