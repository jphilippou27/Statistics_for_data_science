...
There are several instances of very large outliers and we removed the impacted records in an effort to build an extensible model.  The impacted variables include: percent young male (10 pct points higher), density (8.8 compared to 6), police per capita (0.9% compared to 0.5%), and tax revenue per capita (119).
...
```
For the wage figures, most counties followed an almost normal distribution so given the Central Limit Theorm and our sample size above 30, we decided not to transform them. Of course there is always an exception and in this case that is the construction and manufacturing industry which had a stronger right skew so we decided to XX.
```{r, fig.width = 11}
par(mfrow = c(2,5))
