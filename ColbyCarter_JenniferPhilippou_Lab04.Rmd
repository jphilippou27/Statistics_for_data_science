...
test_resids <- data.frame(model1$fitted.values,model1$residuals)
par(mfrow = c(1,2))
plot(model1, which = 1)
plot(model1, which = 5)
...
Compare models using transformed variables:
```{r, fig.width=10}
model1_trans = lm(crmrte ~sqrt(polpc) + sqrt(avgsen) + prbarr + sqrt(prbconv) + prbpris + sqrt(density) + urban + (polpc * urban) +
              (avgsen * urban) + (prbarr * urban) + (prbconv * urban) + (prbpris * urban), data=raw_data)
