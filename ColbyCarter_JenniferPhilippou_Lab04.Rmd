...
# probability of conviction: 0.07%(?) to 2.12%, right skew
...
Build a simple model for eacg variable
```{r}
#check complete list with...
str(df_num_clean, list.len=ncol(df_num_clean))
#df_fv <- df_fv[, !duplicated(colnames(df_fv), fromLast = TRUE)] 
#detach("package:MASS", unload=TRUE) #if the MASS package is installed it messes with dplyr
df_pred <- df_num_clean %>%  select(-year, -county,-X)  # Identifies claims #only one level  

#N=length(df_pred)-2 #number of variables in dataset
...
for(i in (1:length(df_pred))){
  ...
  sig <- add.significance.stars(p) #Add Stars
